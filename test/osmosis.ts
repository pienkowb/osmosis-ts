...
import { decodeMsgSwapExactAmountOut, MsgSwapExactAmountOut, SwapAmountOutRoute } from "../src/osmosis/gamm/v1beta1/tx"
...
export function testOsmosis(): void {
    const msg: MsgSwapExactAmountOut = new MsgSwapExactAmountOut(
        "sender",
        [new SwapAmountOutRoute(3, "tokenInDenom")],
        "tokenInMaxAmount",
        new Coin("denom", "amount"));

    const encoded: Uint8Array = Protobuf.encode<MsgSwapExactAmountOut>(msg, MsgSwapExactAmountOut.encode)
    const decoded_msg = decodeMsgSwapExactAmountOut(encoded);
}